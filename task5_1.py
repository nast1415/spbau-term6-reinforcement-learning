...
def main():
    ...
    grid2 = GridMod234(size_of_grid, cows_array_2, 2)

    alpha = 1
    gamma = 1
    theta = 0.02

    dyna_q_algo(grid2, alpha, gamma, 3000)
    reward = get_policy_result_dyna(grid2)
    print("Grid 10x10 with two cows")
    print("DYNA-Q algorithm")
    print("Summary reward: " + str(reward))
    print("---------------------------------------------------")

    dyna_q_plus_algo(grid2, alpha, gamma, theta, 3000)
    print("start testing")
    reward = get_policy_result_dyna(grid2)
    print("Grid 10x10 with two cows")
